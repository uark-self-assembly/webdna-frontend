...
//var URL_TRJ_AutoLoad_default=PDBDIR+"prueba.pdb";
	...
	// [MRS] This variable is to indicate that a message has been posted to change the scene and the buffers need to be reloaded
	var change_in_frame = false;
    ...
    function drawScene(diffuse)
    {
        ...
        if( (trjbnd && autoplay) || change_in_frame) //trjbnd y autoplay están declaradas en el Main
        {	
		
			//if( change_in_frame ) {window.alert( "Displaying frame number " + pos + " (starting at zero)");}
			...
			var ids = Focus.values();
			var atom_id = ids.next();
			
			while( !atom_id.done )
			{
				// [MRS] Huge assumption here : atm.id is one larger than the index!!
				// The very first atom is ID 1, so need 0th value, i.e. Focus.has(0) SHOULD ALWAYS BE FALSE!!
				
				if(temp <= coordsX.length) {
					x1 += coordsX[atom_id.value+temp-1];
					y1 += coordsY[atom_id.value+temp-1];
					z1 += coordsZ[atom_id.value+temp-1];}
				else {
					x1 += coordsX1[atom_id.value+temp-1];
					y1 += coordsY1[atom_id.value+temp-1];
					z1 += coordsZ1[atom_id.value+temp-1];}
				atom_id = ids.next();
			}
            ...
            for( var i=0; i<molecule.LstBonds.length; i++ )
            {
				...
				if( Focus.has(atm1.id) && Focus.has(atm2.id) ) {
...
//                bndfinal=false;
    ...
    function PauseTraj()
    {
        return function (event)
        {
			...
			// [MRS] Send message to parent to display this frame number in the bonds canvas
			// numframe is synced bonds canvas <-> htmol as starting at zero (not the number displayed in htmol)
			parent.postMessage( numframe, '*' );
        }
    }

    
function RewFor(op, jump = null) {
    return function(event) {
        if (op == 1) {
            if (pos > 0) {
                pos--;
		...
		// [MRS] code to jump to specific frames whenever graph is clicked
		if( pos != numframe ) {
			console.log( "Developers Note : pos and numframe are not equal, probably shouldn't set both to \"jump\" variable" );
			console.log( "op = " + op + "\tpos = " + pos + "\tnumframe = " + numframe + "\tjump = " + jump );}
		
		if( op == null && jump != null ) {
			console.log( "Jumping to frame : " + jump );
			pos = parseInt(jump);
			numframe = parseInt(jump);}
 ...
 /**/
    ...
    function manejadorEventoWorker1(event)
    {
        ...
        else if(event.data.cmd=="endfinal")
        {
			...
			// [MRS] send message that trajectory file is loaded
			parent.postMessage( 'Trajectory File Loaded','*' );
	    }
        else
        {
            requireddata=true;
