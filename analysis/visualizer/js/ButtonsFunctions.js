...
function ChangeFocus( highlighted ) {
	
	//console.log(highlighted);
	
	Focus.clear();
	highlighted[0].forEach( atom => {if(!isNaN(parseInt(atom))){Focus.add( parseInt(atom) );}} );
	
	var red_set = new Set( highlighted[1] );
	var red_list = [];
	var blue_set = new Set( highlighted[2] );
	var blue_list = [];
	var green_set = new Set( highlighted[3] );
	var green_list = [];
	
	//console.log( Array.from(Focus.values()) );
	
	var selection = [];
	var notselect = [];
	for( var i = 0; i < molecule.LstAtoms.length; i++ ) {
		if( Focus.has(molecule.LstAtoms[i].id) ) {
			selection.push(molecule.LstAtoms[i]);
			if( red_set.has(molecule.LstAtoms[i].id) ) {
				red_list.push(molecule.LstAtoms[i]);}
			if( blue_set.has(molecule.LstAtoms[i].id) ) {
				blue_list.push(molecule.LstAtoms[i]);}
			if( green_set.has(molecule.LstAtoms[i].id) ) {
				green_list.push(molecule.LstAtoms[i]);}}
		else {notselect.push(molecule.LstAtoms[i]);}}
	
	//[MRS] molecule.LstAtoms[0].id = 1!! ID IS ONE LARGER THAN THE INDEX!!
	
	AtomosSeleccionados = selection;
	CambiarRepresentacion('SB');
	CambiarColor('atom');
	
	if( red_list.length > 0 ) {
		AtomosSeleccionados = red_list;
		CambiarRepresentacion('VDW');
		CambiarColor('red');}
	
	if( blue_list.length > 0 ) {
		AtomosSeleccionados = blue_list;
		CambiarRepresentacion('VDW');
		CambiarColor('blue');}
	
	if( green_list.length > 0 ) {
		AtomosSeleccionados = green_list;
		CambiarRepresentacion('VDW');
		CambiarColor('green');}
	
	AtomosSeleccionados = notselect;
	CambiarRepresentacion('SB');
	CambiarColor('transparent');
	
	AtomosSeleccionados = [];
	change_in_frame = true;
	drawScene(0);
}
...
function CambiarColor(color) //función para cambiar el color
{
    ...
    for(var i=0; i<AtomosSeleccionados.length; i++)
    {
        ...
        for (var z = 0; z < nColor;) 
        {
			if( ColorTotal[atom.BloqueSolid-1] == null ){console.log(atom.BloqueSolid-1);}
			if( isNaN(mul + z) ){console.log(mul);}
