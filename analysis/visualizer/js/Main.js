...
function Main() {
	...
	Focus = new Set();
	for( var i = 1; i <= molecule.LstAtoms.length; i++ ) {Focus.add(i);}
    ...
    this.Parse = function (txt) // Parsing commands in the console
    {
		...
		} else if (comando=='focus') /*	[MRS] make a Focus attribute that holds the indicies of atoms
										that camera will center around, used in MainHTMoL script drawScene() */
		{
			if( inst == "all" ) {
				...
				inst1 = parseInt(inst.substr( 0, inst.indexOf(":") ));
				inst2 = parseInt(inst.substr( inst.indexOf(":")+1, inst.length ));
				
				if( 0 < inst1 && inst1 <= inst2 && inst2 <= molecule.LstAtoms.length ) {
					Focus.clear();
					for( var i = inst1; i <= inst2; i++ ) {Focus.add(i);} }
				else {
					console.log( "ERROR : Invalid indicies, must be greater than 0 and less than or equal to length!!" );}
